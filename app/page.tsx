...
import { type HomepageSection, DEFAULT_BENEFITS } from '@/lib/learn/homepage';
...
import CreatorPills from '@/components/learn/CreatorPills';
...
export default async function HomePage() {
  ...
  // "Top Creators" shows every active creator (ordered by sort_order), not only
  // the is_featured ones — so the section never collapses to a single card.
  const creators = activeCreators;
  const playlists =
  ...
  function renderSection(s: HomepageSection) {
    switch (s.type) {
      ...
      case 'value_props': {
        const items = s.items && s.items.length > 0 ? s.items : DEFAULT_BENEFITS;
        return (
