import Link from 'next/link';
import { listContent } from '@/lib/learn/db';
import { Plus, BookOpen } from 'lucide-react';
import ImportLegacyButton from './ImportLegacyButton';
import DuplicateCourseButton from './DuplicateCourseButton';
import DeleteRowButton from '@/components/learn-admin/DeleteRowButton';

export const metadata = { title: 'ניהול קורסים — Digitech Learning Hub' };
export const dynamic = 'force-dynamic';

function formatDate(iso: string | null) {
  if (!iso) return '—';
  try {
    return new Intl.DateTimeFormat('he-IL', { dateStyle: 'short' }).format(new Date(iso));
  } catch {
    return iso;
  }
}

export default async function CoursesAdminIndex() {
  const items = await listContent('course');

  return (
    <div className="px-8 py-8 max-w-5xl">
      <header className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-extrabold text-neutral-950">קורסים</h1>
          <p className="text-sm text-neutral-500 mt-1">סדרות שיעורים עם וידאו, חומרים ופלייבוקים אוטומטיים.</p>
        </div>
        <div className="flex items-center gap-2">
          <ImportLegacyButton />
          <Link
            href="/admin/courses/new"
            className="flex items-center gap-2 px-4 py-2 rounded-pill bg-brand-purple-700 hover:bg-brand-purple-600 text-white text-sm font-semibold transition-colors"
          >
            <Plus className="w-4 h-4" /> קורס חדש
          </Link>
        </div>
      </header>

      {items.length === 0 ? (
        <div className="bg-white rounded-2xl border border-neutral-200 p-12 text-center">
          <div className="w-12 h-12 mx-auto mb-3 rounded-pill bg-brand-purple-50 flex items-center justify-center text-brand-purple-600">
            <BookOpen className="w-6 h-6" />
          </div>
          <h3 className="font-extrabold text-neutral-900 mb-1">עוד אין קורסים</h3>
          <p className="text-sm text-neutral-500 mb-4">
            צור קורס חדש, או ייבא קורסים קיימים מהמערכת הישנה.
          </p>
        </div>
      ) : (
        <div className="bg-white rounded-2xl border border-neutral-200 overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-neutral-50 text-neutral-500 text-xs uppercase">
              <tr>
                <th className="text-right px-4 py-3 font-semibold">כותרת</th>
                <th className="text-right px-4 py-3 font-semibold">סטטוס</th>
                <th className="text-right px-4 py-3 font-semibold">מנוי</th>
                <th className="text-right px-4 py-3 font-semibold">עודכן</th>
                <th className="px-4 py-3" />
              </tr>
            </thead>
            <tbody>
              {items.map((c) => (
                <tr key={c.id} className="border-t border-neutral-100 hover:bg-neutral-50">
                  <td className="px-4 py-3">
                    <Link href={`/admin/courses/${c.slug}`} className="font-semibold text-neutral-900 hover:text-brand-purple-700">
                      {c.title}
                    </Link>
                    {c.tagline && <p className="text-xs text-neutral-500 mt-0.5 truncate max-w-md">{c.tagline}</p>}
                  </td>
                  <td className="px-4 py-3">
                    <span
                      className={[
                        'inline-block px-2 py-0.5 rounded-pill text-[11px] font-semibold',
                        c.status === 'published' ? 'bg-emerald-100 text-emerald-800' : 'bg-neutral-200 text-neutral-700',
                      ].join(' ')}
                    >
                      {c.status === 'published' ? 'פורסם' : 'טיוטה'}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <span
                      className={[
                        'inline-block px-2 py-0.5 rounded-pill text-[11px] font-semibold',
                        c.is_premium ? 'bg-brand-purple-100 text-brand-purple-800' : 'bg-neutral-100 text-neutral-600',
                      ].join(' ')}
                    >
                      {c.is_premium ? 'פרימיום' : 'חינמי'}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-neutral-500 text-xs">{formatDate(c.updated_at)}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      <DuplicateCourseButton slug={c.slug} />
                      <DeleteRowButton
                        endpoint={`/api/content/course/${c.slug}`}
                        label={c.title}
                      />
                    </div>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      )}
    </div>
  );
}
